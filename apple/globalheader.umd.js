...
var sr = "",
			...
			return {
				...
				set: (e, t, s) => {
					...
					if (n !== s) {
						...
						Reflect.set(e, t, a ? this._proxifyReference(s, i.split(".")) : s), Object.keys(this.autoWatchedProps).includes(i) && this._toggleMixins(i), Object.keys(this.options.watch).includes(i) && this.__triggerEvent(this.options.watch[i], {
							from: n,
							to: s
						})
					}
	...
	pi = {
		_toggleMixins(e) {
			...
			for (let n of s.reverse()) n.destroy && n.destroy.call(this.options.ref);
			this._enabledMixins = this._getEnabledMixins();
			for (let n of t) {
				n._isEnabled = !0;
				for (let a of this.options.setupEvents) n[a] && n[a].call(this.options.ref)
			}
		},
		_getEnabledMixins() {
			const e = [];
		...
		_setupViewportEvents(e, t, s) {
			if (!!this.options.viewportEvents) {
				...
				for (const n of this.options.breakpoints) {
					...
					n.viewportQuery = window.matchMedia(i), n.viewportQuery.matches && (this.options.ref._viewportInfo.viewport = a), n._viewportChangeHandler = this._viewportChangeHandler.bind(this, a), n.viewportQuery.addEventListener("change", n._viewportChangeHandler)
				}
...
class Pt {
	constructor(t, ...s) {
		return Object.assign(Pt.prototype, di, gi, fi, hi, pi, mi), this.options = Object.assign({}, Ei, t), this.events = [...new Set(this.options.events.concat(Object.values(this.options.watch), wi))], this.mixinsList = [...new Set(s.flat(9999).map(n => n.default && n.default() || typeof n == "function" && n() || n))], this.isDestroyed = !1, this.queue = [], this.autoWatchedProps = {}, this.currentMixin = null, this._enabledMixins = [], this._setupViewportEvents(Ni, yi, Si), this._attachEvents(), this._attachMethods(), this._attachDefaultMethods(Li), this.options.ref = this._proxifyReference(this.options.ref, []), this._toggleMixins(), this.options.ref
	}
}
var Ci = {
		onViewportChange(e) {
	...
	Ai = typeof globalThis != "undefined" ? globalThis : typeof window != "undefined" ? window : typeof global != "undefined" ? global : typeof self != "undefined" ? self : {};
...
at.isFocusableElement = function(e, t, s) {
	...
	return n ? !e.disabled : e.contentEditable ? (s = s || parseFloat(e.getAttribute("tabindex")), !isNaN(s)) : !0
	...
	for (var s = e.querySelectorAll(this.focusableSelectors), n = s.length, a = [], i = 0; i < n; i++) this.isTabbableElement(s[i], t) && a.push(s[i]);
	return a
}, at.getFocusableElements = function(e, t) {
	for (var s = e.querySelectorAll(this.focusableSelectors), n = s.length, a = [], i = 0; i < n; i++) this.isFocusableElement(s[i], t) && a.push(s[i]);
	return a
};
var Dt = new Ps,
	Ds = {
	...
	Ht = function(e, t) {
		...
		s || (s = e.getAttribute(t) || "", e.setAttribute(Us + t, s))
	},
	$i = function(t, s) {
		if (Ws.isFocusableElement(t, s)) Ht(t, bt), t.setAttribute(bt, "-1");
		else
			for (var n = Ws.getTabbableElements(t, s), a = n.length; a--;) Ht(n[a], bt), n[a].setAttribute(bt, "-1");
		Ht(t, Hs.HIDDEN), t.setAttribute(Hs.HIDDEN, "true")
	},
	Vs = $i,
	Ti = function e(t, s, n) {
		s = s || document.body;
		for (var a = t, i = t; a = a.previousElementSibling;) Vs(a, n);
		for (; i = i.nextElementSibling;) Vs(i, n);
		t.parentElement && t.parentElement !== s && e(t.parentElement, s, n)
	},
	Oi = function(e, t) {
		let s;
		e instanceof NodeList ? s = e : s = [].concat(e), t = Array.isArray(t) ? t : [].concat(t), s.forEach(n => {
			t.forEach(a => {
				n.removeAttribute(a)
			})
		})
	...
	Vt = function(e, t) {
		...
		s !== null && (s === "" ? qs(e, t) : e.setAttribute(t, s), qs(e, Wt + t))
	},
	Ri = function(t) {
		Vt(t, Ut), Vt(t, xi.HIDDEN);
		for (var s = t.querySelectorAll(`[${Wt+Ut}]`), n = s.length; n--;) Vt(s[n], Ut)
	},
	Zs = Ri,
	Bi = function e(t, s) {
		s = s || document.body;
		for (var n = t, a = t; n = n.previousElementSibling;) Zs(n);
		for (; a = a.nextElementSibling;) Zs(a);
	...
	ot = js.prototype;
ot.start = function(e) {
	this.updateTabbables(), Di(this.el, null, this._excludeHidden);
	...
	Hi(this.el), document.removeEventListener("focus", this._handleOnFocus, !0)
}, ot.updateTabbables = function() {
	this._tabbables = Pi.getTabbableElements(this.el, this._excludeHidden), this._firstTabbableElement = this._firstTabbableElement || this._tabbables[0], this._lastTabbableElement = this._tabbables[this._tabbables.length - 1]
}, ot._handleOnFocus = function(e) {
	if (this.el.contains(e.target)) this._relatedTarget = e.target;
	else {
		if (e.preventDefault(), this.updateTabbables(), this._relatedTarget === this._lastTabbableElement || this._relatedTarget === null) {
			this._firstTabbableElement.focus(), this._relatedTarget = this._firstTabbableElement;
			return
		}
		if (this._relatedTarget === this._firstTabbableElement && this._lastTabbableElement) {
			this._lastTabbableElement.focus(), this._relatedTarget = this._lastTabbableElement;
			return
		}
	}
}, ot.destroy = function() {
...
var Wi = js,
	...
	yt = {
		...
		onFlyoutWillClose(e, t) {
			...
			}, 300)), this._isBreakpointWithMenu() || (s = this._lastFlyoutTrigger) == null || s.setAttribute("aria-expanded", !1), t || (!this._isBreakpointWithMenu() && this._getFlyout(e).classList.remove(this.classNames.flyoutShort), (a = (n = this.menuButtonAnimation.close.top).beginElement) == null || a.call(n), (l = (i = this.menuButtonAnimation.close.bottom).beginElement) == null || l.call(i)), this.circTab.stop()
		},
		onFlyoutOpen(e, t) {
		...
		initiateListeners() {
			const e = new AbortController,
				...
				s = !0,
				n = !0;
			this.el.addEventListener("focusout", this._onTopNavFocusOut), this._isBreakpointWithMenu() || (document.addEventListener("mousemove", a => {
		...
		initiateFlyouts() {
			...
			for (const a of e) {
				let i = {};
				i.item = a, i.flyout = (t = i.item) == null ? void 0 : t.querySelector(`.${this.classNames.flyout}`), i.scrollContainer = (s = i.flyout) == null ? void 0 : s.querySelector(`:scope > .${this.classNames.flyoutScrollContainer}`), i.content = (n = i.scrollContainer) == null ? void 0 : n.querySelector(`:scope > .${this.classNames.flyoutContent}`), i.trigger = i.content && {
					regular: a.querySelector("[data-topnav-flyout-trigger-regular]"),
					compact: a.querySelector("[data-topnav-flyout-trigger-compact]")
				}, this.flyouts.push(i)
			}
			this.setFlyoutItems(), this._setFlyoutHeights(), this.circTab = new Wi(this.el)
		},
		setFlyoutItems() {
			const e = (t, s = 0) => {
				...
				}), t.style.setProperty(this.cssVariables.flyoutItemTotal, n.length), t.style.setProperty(this.cssVariables.flyoutGroupNumber, s)
			};
		...
		pagehide(e) {
			...
			!t || (this.el.classList.add(this.classNames.blockTransitions), (s = this.onFlyoutWillClose(t).onFlyoutClose(t)) == null || s.closeMenu(), this.el.classList.remove(this.classNames.blockTransitions))
		},
		destroy() {
		...
		_checkFlyoutHeight(e) {
			...
			window.innerHeight - a < i ? (s.classList.add(this.classNames.flyoutShort), (h = this.flyoutAbortController) == null || h.abort()) : s.classList.remove(this.classNames.flyoutShort), s.style.setProperty(this.cssVariables.scrollbarWidth, `${l}px`)
		},
		_listenForAnimationChangeEnd(e, t, s, n) {
			const a = new AbortController,
				...
				h = ({
					...
					[c === `${this.options.className}-scrim-height-change`, c === `${this.options.className}-flyout-slide-forward-next`, c === `${this.options.className}-flyout-slide-back-next`, f === "height" && l, f === "height" && t === this.content].some(b => !!b) && (a.abort(), n(e))
				};
		...
		_setFlyoutHeight(e, t = 0) {
			...
			if (this._isBreakpointWithMenu()) {
				...
				if (a) {
					...
					i.style.setProperty(this.cssVariables.flyoutHeight, `${l+h+t}px`), i.style.setProperty(this.cssVariables.flyoutRateOfChange, s(l + t))
				}
			}
		},
		_setFlyoutHeights() {
		...
		_onFlyoutDocumentKeyDown(e) {
			...
			switch (t) {
				case re.ESCAPE:
					...
					this.closeFlyout(), l === h && this._flyoutTimeouts.push(setTimeout(() => {
						i.focus()
					}, 300));
					break
			}
	...
	Vi = [yt, {
		...
		onMenuButtonClick(e) {
			...
			if (this._isFlyoutActiveClick = !0, this.el.classList.contains(this.classNames.withMenuOpen)) this.closeMenu();
				...
				this.menu.elements.menuButton.blur(), this._flyoutFocusTimeout = setTimeout(() => {
	...
	qi = [yt, {
		...
		onMenuItemMouseOver({
			...
			}, s), this.el.addEventListener("mouseleave", this.onMenuItemMouseLeave, {
				once: !0
		...
		onMenuItemButtonClick(e) {
			...
			if (!!t.closest(`.${this.classNames.subMenuTriggerButton}`) || s && this._usesTouchEvents) {
				...
				l !== h && c && e.preventDefault(), i ? this.closeFlyout(this._currentFlyout) : this.openFlyout(a)
			}
		},
		...
		initiateSubMenus() {
			this.submenus = this.flyouts.filter(e => e.item.classList.contains(this.classNames.itemSubMenu)).map(e => {
				const {
					item: t
				} = e, s = t.querySelector(`.${this.classNames.subMenuContent}`), n = t.querySelector(`.${this.classNames.subMenuTriggerGroup}`), a = t.querySelector(`.${this.classNames.subMenuTriggerLink}`), i = a.getAttribute("aria-label"), l = t.querySelector(`.${this.classNames.subMenuTriggerButton}`), h = l.getAttribute("aria-label"), c = {
					group: n,
					link: a,
					linkLabel: i,
					button: l,
					...
					const b = r.classList.contains(this.classNames.subMenuGroupElevated),
						y = r.querySelector(`.${this.classNames.subMenuHeader}`),
		...
		_setSubMenuItemCounts() {
			...
			this.submenus.forEach(t => {
				...
				e || (i = 0), n.forEach((h, c) => {
					...
					y.style.setProperty(this.cssVariables.flyoutGroupNumber, c), e || (l = 0), [r, ...b.children].forEach((o, g) => {
						o.style.setProperty(this.cssVariables.flyoutItemNumber, ++l), (!c || e) && ++i
					}), f && ++a
				}), s.style.setProperty(this.cssVariables.flyoutElevatedGroupCount, a), s.style.setProperty(this.cssVariables.flyoutGroupTotal, n.length - 1), s.style.setProperty(this.cssVariables.flyoutItemTotal, i)
			})
		},
		_addSubMenuEvents() {
	...
	Zi = function(t) {
		const s = {};
		for (const n in t) s[n] = t[n].reduce(function(a, i) {
			return a[i.name] = !1, a
		}, {
			version: {
				string: "",
	...
	ji = {
		...
		os: [{
			...
			name: "ios",
			test: function(e) {
				return e.ua.indexOf("mela") > -1 || e.ua.indexOf("iPad") > -1
			},
			version: ["mela OS", "CPU OS"]
		}, {
			name: "linux",
			userAgent: "Linux",
			test: function(e) {
...
function Ki(e, t) {
	if (typeof e.parseVersion == "function") return e.parseVersion(t);
	{
		...
		let a;
		for (let i = 0; i < n; i++)
			if (a = t.match(zi(s[i])), a && a.length > 1) return a[1].replace(/_/g, ".")
	}
	return !1
}
...
function zs(e, t, s) {
	...
	let a, i;
	for (let l = 0; l < n; l++)
		if (typeof e[l].test == "function" ? e[l].test(s) === !0 && (a = e[l].name) : s.ua.indexOf(e[l].userAgent) > -1 && (a = e[l].name), a) {
			if (t[a] = !0, i = Ki(e[l], s.ua), typeof i == "string") {
				const h = i.split(".");
				t.version.string = i, h && h.length > 0 && (t.version.major = parseInt(h[0] || 0), t.version.minor = parseInt(h[1] || 0), t.version.patch = parseInt(h[2] || 0))
			} else a === "edge" && (t.version.string = "12.0.0", t.version.major = "12", t.version.minor = "0", t.version.patch = "0");
			return typeof e[l].parseDocumentMode == "function" && (t.version.documentMode = e[l].parseDocumentMode()), t
		} return t
}
...
function Qi(e) {
	const t = {},
		s = Gi(qt);
	return t.browser = zs(qt.browser, s.browser, e), t.os = zs(qt.os, s.os, e), t
}
var Yi = Qi;
...
var ea = function() {
	...
	sa = {
		...
		onTextZoomResize(e) {
			...
			this.el.classList[s <= 1 ? "remove" : "add"](this.classNames.textZoomIn), this.el.style.setProperty(this.cssVariables.textZoomScale, s)
		},
		destroy() {
	...
	ia = function e(t) {
		if (typeof t != "object" || t === null) throw new TypeError("object-utils.deepClone : Invalid parameter - expected object");
		var s = Array.isArray(t) ? [] : {};
		for (var n in t) na.call(t, n) && (typeof t[n] == "object" && t[n] !== null ? s[n] = e(t[n]) : s[n] = t[n]);
		return s
	};
...
var aa = function e(t, s, n) {
		...
		for (var r = 0; r < h; r++) {
			...
			if (typeof b == "object" && typeof y == "object") {
				var o = e(b, y, n);
				if (typeof o != "boolean" && (o.key = "[" + a + "]" + o.key), o !== !0) return o
			} else if (b !== y) return Gt(n, a, b, y)
		}
		return !0
	},
	oa = Object.prototype.hasOwnProperty,
	ra = function(t) {
		if (typeof t != "object") throw new TypeError("object-utils.isEmpty : Invalid parameter - expected object");
		for (var s in t)
			if (oa.call(t, s)) return !1;
		return !0
	},
	Ks = {
		deepClone: ia,
	...
	la = [Gs, {
			...
			onFlyoutWillOpen(e) {
				e === this.search.elements.el && (this.clearSearch(), this.search.state.isOpening = !0, this.search.state.activeState = 0, this.search.initialResults ? this._renderSearchResults(this.search.templateFunction(this.search.initialResults)) : this.onSearchUpdate(), this._addSearchAnimationEvents())
			},
			onFlyoutOpen(e) {
			...
			onFlyoutClose(e) {
				...
				this.search.elements.el.classList.forEach(t.bind(this, this.search.elements.el)), [...this.search.elements.results.querySelectorAll(`.${this.classNames.searchResultsContainer}`)].forEach((s, n) => {
					n && s.remove()
			...
			onSearchInputKeyDown(e) {
				...
				switch (t) {
					...
					case re.ARROW_DOWN:
						...
						break;
					case re.ENTER:
						this.search.elements.input.value.trim() === "" && this.search.selectionController.active === -1 ? (e.preventDefault(), e.stopPropagation()) : this.search.selectionController.active > -1 ? this.search.selectionController.links[this.search.selectionController.active].click() : this.onSearchInputChange(e);
						break;
					case re.ESCAPE:
						e.preventDefault();
						break;
					default:
						...
						clearTimeout(this.search.searchDebounce), this.search.searchDebounce = setTimeout(() => this.onSearchInputChange(e), n)
				}
			},
			onSearchInputChange(e) {
				...
				const {
					keyCode: t
				} = e, s = ((i = (a = e.target) == null ? void 0 : a.value) == null ? void 0 : i.trim()) || "";
				if (s.length < 2 && !this.search.previousSearchTerm || this.search.previousSearchTerm === s || t === re.ENTER) {
					this._updateSearchFieldButtons();
					return
				}
				this.search.previousSearchTerm = s, this.onSearchUpdate(e)
			},
			onSearchResultsRendered() {
				var a, i;
				const {
					value: e
				} = this.search.elements.input, t = (a = this.search.elements.results) == null ? void 0 : a.querySelectorAll(`.${this.classNames.searchResultsCurrent} .${this.classNames.searchResultsContainer}`).length, n = `${(i=this.search.elements.results)==null?void 0:i.querySelectorAll(`.${this.classNames.searchResultsCurrent} .${this.classNames.searchResultsListItem}`).length} ${this.search.state.resultsLabel}`;
				this._flyoutTimeouts.push(setTimeout(() => {
					this.search.elements.count.textContent = n, this._flyoutTimeouts.push(setTimeout(() => {
						this.search.elements.count.textContent = ""
					}, this.search.state.countResetDelayTime))
				}, this.search.state.countSetDelayTime)), this._updateSearchFieldButtons(), this.search.elements.el.classList[e.length > 1 ? "add" : "remove"](this.classNames.searchWithResults), this.search.elements.el.classList[t ? "remove" : "add"](this.classNames.searchNoResults), this._setFlyoutHeight(this.search.elements.el, this._getSearchFlyoutHeightOffset())
			},
			blurSearchInput(e) {
			...
			setSearchResults(e = []) {
				...
				if (t && !this.search.elements.cachedResults) {
					...
					this.search.elements.el.classList.add(this.classNames.searchResultsOverrideHighlight), this._updateSearchFieldButtons();
					return
				} else if (s) {
					const {
						cachedSearchValue: l
						...
						value: h
					} = this.search.elements.input, c = !(l.includes(h) || h.includes(l));
					this.search.elements.el.classList[c ? "add" : "remove"](this.classNames.searchResultsOverrideHighlight)
				} else this.search.elements.cachedResults = ((a = (n = this.search.elements.input) == null ? void 0 : n.value) == null ? void 0 : a.length) > 1 ? e : null, this.search.elements.el.classList.remove(this.classNames.searchResultsOverrideHighlight);
				this.search.cachedSearchValue = (i = this.search.elements.input) == null ? void 0 : i.value, this._renderSearchResults(this.search.templateFunction(e), s)
			},
			_addSearchEvents() {
			...
			_onSearchAnimationEnd() {
				var e;
				this.search.state.activeState || this.search.elements.results.container.remove(), (e = this.search.elements.results.containerPrevious) == null || e.remove(), this.el.classList.remove(this.classNames.animating), this.search.state.isAnimating = !1, this.search.state.lastAnimatedItem = null
			},
			...
			_getSearchFlyoutHeightOffset() {
				const {
					marginTop: e
				} = getComputedStyle(this.search.elements.results), t = this.search.elements.el.classList.contains(this.classNames.searchNoResults);
				let s = 0;
			...
			_setSearchItemNumbers(e) {
				...
				l.forEach((c, f) => c.style.setProperty(this.cssVariables.flyoutGroupNumber, f)), h.forEach((c, f) => c.style.setProperty(this.cssVariables.flyoutItemNumber, f)), this.search.elements.flyout.style.setProperty(this.cssVariables.flyoutItemTotal, h.length - 1)
			},
			...
			_initSearchResultsSelectionController() {
				var e;
				this.search.selectionController = {}, this.search.selectionController.links = [], this.search.selectionController.active = -1, this.search.selectionController.links = ((e = this.search.elements.results.container) == null ? void 0 : e.querySelectorAll(`.${this.classNames.searchResultsListLink}`)) || []
			},
			_createSearchResultsTemplate(e) {
				...
				return `${e.map(n=>{const{title:a,results:i,icon:l}=n;return`				<div class="${this.classNames.searchResultsContainer}">
				...
				return t.classList.add(this.classNames.searchResultsCurrent), t.innerHTML = e, this.search.elements.currentResults = t, t
				...
				if (this.search.state.activeState === 2) {
					const [f, r] = this.search.elements.results.container.querySelectorAll(`.${this.classNames.searchResultsContainer}`), b = f.scrollHeight, {
						marginTop: y
					} = getComputedStyle(r), o = parseInt(y);
					this.search.elements.flyout.style.setProperty(this.cssVariables.searchSuggestedLinksHeight, `${b+o}px`)
				}
				this.search.elements.el.classList.remove(this.search.previousShift), requestAnimationFrame(() => {
					this.search.elements.el.classList.add(this.search.currentShift), this._setFlyoutHeight(this.search.elements.el, this._getSearchFlyoutHeightOffset()), this._setSearchItemNumbers(), this.onSearchResultsRendered(), t && this._onSearchAnimationEnd()
				})
				}
				}];
				const ca = {
				...
				class ua {
					constructor(t, s = {}) {
						this.el = t, this.options = Object.assign({}, ca, s), this._viewportsWithMenu = ["xsmall", "small", "medium"];
						const n = {
							ref: this,
						...
						}, new Pt(n, Ci, ...this.options.mixins), this.options.manuallyInitLifecycles ? this : this.beforeCreate().created().beforeMount().mounted()
					}
				}
				const Qs = gt(null),
					ha = Qs.Provider,
				...
				function ba(e, t, s, n) {
					const a = e.target;
					s.current == null && (s.current = n.current), a.setAttribute(t, s.current), n.current = s.current, s.current = null
				}
				...
				const Ea = (e, t) => {
					...
					Ee = e => {
						if (typeof e != "undefined") return e.reduce((t, s) => (t[s.name] = s.value, t), {})
					},
					me = (e, t) => {
						typeof t != "undefined" && typeof e != "undefined" && t.forEach(({
							name: s,
							value: n
						}) => {
							e.setAttribute(s, n)
				...
				var La = {
						...
						created() {
							this.amlSearch = {}, this.amlSearch.requestId = "", this.amlSearch.requestType = "";
							const {
								hasAbsoluteUrls: e,
							...
							this.amlSearch.apiRequestDomain = e && !t ? `https://${s}` : "", this.amlSearch.locale = n.searchFieldLocale || a, n.searchSuggestionsEnabled !== !1 ? this.search.templateFunction = this._createAMLSearchTemplate : this.search.templateFunction = this._createAMLSearchDisabledTemplate
						},
						beforeMount() {
							this.search.elements.el.setAttribute(this.dataAttributes.iconFlyoutEnabled, !0), me(this.search.elements.el, this.options.amlSearch.searchSettings.analyticsAttributes), St(this.search.elements.el, this.options.amlSearch.searchSettings.open.analyticsAttributes), this.options.amlSearch.searchSettings.searchSuggestionsEnabled !== !1 ? wa({
								...
								this.setInitialSearchResults(n), this._setPlaceHolderText(), me(this.search.elements.link, this.options.amlSearch.searchSettings.open.analyticsAttributes)
							}).catch(() => {}) : this.setInitialSearchResults()
						},
						onLayoutChange() {
							me(this.search.elements.link, this.options.amlSearch.searchSettings.open.analyticsAttributes), this._setPlaceHolderText()
						},
						onFlyoutOpen(e) {
							e === this.search.elements.el && (St(this.search.elements.link, this.options.amlSearch.searchSettings.open.analyticsAttributes), this._isBreakpointWithMenu() && this.menu.elements.menuButton.setAttribute("aria-label", this.options.amlSearch.searchSettings.close.ariaLabel))
						},
						onFlyoutClose(e) {
							e === this.search.elements.el && (me(this.search.elements.link, this.options.amlSearch.searchSettings.open.analyticsAttributes), this.search.elements.form.setAttribute("action", this.options.amlSearch.searchSettings.searchFieldAction))
						},
						onSearchUpdate(e) {
							if (!e && this.options.amlSearch.searchSettings.searchSuggestionsEnabled === !1) return;
							const {
								value: t
							} = this.search.elements.input;
							t.length >= 2 ? (this.amlSearch.requestType = "suggestions", Na({
								query: t,
							...
							}).catch(() => {})) : (this.amlSearch.requestType = "defaultlinks", this.setSearchResults(this.search.initialResults))
						},
						_setPlaceHolderText() {
						...
						_createAMLSearchTemplate(e) {
							...
							return `${e.map(s=>{const{title:n,icon:a,analyticsAttributes:i,results:l}=s,h=this.amlSearch.requestType==="defaultlinks"?"quickLinks":s.sectionName==="quickLinks"?"suggestedLinks":"suggestedSearches",c=i.map(f=>`
							$ {
								f.name
							} = "${f.value}"
							`).join("");return` < div					class="${this.classNames.searchResultsContainer}"
					${c}
				>
			...
			const Xa = ({
				...
				var r, b;
				const [l, h] = se([]), c = it(Qs), f = e == null ? void 0 : e.reduce((y, o) => (Qa.includes(o.name) ? y.nonMenuItems.push(o) : y.menuItems.push(o), y), {
					nonMenuItems: [],
					menuItems: []
				});
				return ve(() => {
					let y = !1;
					return Ya(s, n).then(Ja).then(o => {
						y || h(o)
					}).catch(() => {}), () => {
						y = !0
					}
					...
					children: [(r = f == null ? void 0 : f.nonMenuItems) == null ? void 0 : r.map(y => {
						const {
							id: o,
							name: g,
							text: k,
						...
						children: m("div", {
							...
							children: m("div", {
								...
								children: (b = f == null ? void 0 : f.menuItems) == null ? void 0 : b.map(y => {
									const {
										id: o,
										name: g,
										text: k,
				...
				return ve(() => {
					...
					children: [m("div", {
						...
						children: [m(to, {
							...
							type: "button",
							onClick: S,
							children: (N = l.images) == null ? void 0 : N.map(x => {
								const {
									name: D,
									assetInline: z
								} = x;
							...
							"aria-hidden": "true",
							type: "submit",
							children: (A = i.images) == null ? void 0 : A.map(x => {
								const {
									name: D,
									assetInline: z
								} = x;
			...
			const co = "bagitem-adding",
				uo = "bagitem-removing",
				rt = "with-bag-count",
				ho = "with-bag-count-onload",
				mo = `${rt}-double`,
				po = `${rt}-triple`;
			...
			function fo({
				...
				analyticsType: b,
				wwwDomain: y = "www.apple.com",
				hasAbsoluteUrls: o = !1,
				useRelativeSearchRequest: g = !1,
				hasShopRedirectUrls: k = !0,
				subMenuData: T,
				setAcStoreInstance: I
			}) {
				var Ct;
				typeof o == "string" && (o = Rt(o)), typeof g == "string" && (g = Rt(g));
				const S = G(null),
					N = G(null),
					A = G(void 0),
					x = G(void 0),
				...
				Ea(S), ve(() => {
					...
					Pe && c && (R.mixins.push(la, La), c.searchFieldSrc = z.current, c.searchFieldAction = D.current !== void 0 ? D.current : ke(c.open.url, o === !0 ? y : void 0), c.searchFieldLocale = X.current, c.searchFieldPlaceholderRegular = B.current !== "" ? B.current : c.input.placeholderTextRegular, c.searchFieldPlaceholderCompact = de.current !== "" ? de.current : c.input.placeholderTextCompact, c.searchAriaLabel = B.current !== "" ? B.current : c.open.ariaLabel, c.searchSuggestionsEnabled = pe.current !== void 0 ? pe.current : void 0, R.amlSearch = {
						searchSettings: c,
						locale: e,
						wwwDomain: y,
						hasAbsoluteUrls: o,
						useRelativeSearchRequest: g
					}), xe && x.current !== void 0 && A.current !== !1 && (R.mixins.push(Ca), R.bagData = {
						data: n,
						bagHref: `${f}/shop/goto${n.open.url}`
					});
					const W = new ua(ae, R);
					return Ne(W), ae !== null && Oe(!0), () => {
						ae !== null && (Oe(!1), W.destroy(), Ne(null))
					}
				}, [n, o, g, xe, Pe, e, a, c, f, y]), ve(() => {
					const ae = S == null ? void 0 : S.current,
						R = N == null ? void 0 : N.current,
						W = Q => {
							fe === 0 && Q !== 0 && Ue(ho)
						},
						qe = Q => {
							Q === 0 ? be("") : Q > 0 && Q < 10 ? be(rt) : Q >= 10 && Q < 100 ? be(`${rt} ${mo}`) : Q >= 100 && be(`${rt} ${po}`)
						},
						Ze = Q => {
							Ue(""), Q > 0 && fe === 0 ? Te(co) : fe > 0 && Q === 0 ? Te(uo) : Te("")
						};
					return R !== null && x.current !== void 0 && A.current !== !1 && Promise.all([Promise.resolve().then(function() {
						return wn
					})]).then(Q => {
						let [{
							AcStore: je,
						...
						_ === void 0 && (_ = new je(R, ue, x.current, Ge), _ !== void 0 && (_.getStoreState().then(Y => {
							_ == null || _.getItemCount().then(ee => {
								W(ee), qe(ee), Le(ee)
							}), _ == null || _.on("itemCountChange", ee => {
								qe(ee), Ze(ee), Le(ee)
							})
						}, Y => {
							window.acStore.isDisabled = !0, S.current !== null && (S == null || S.current.addEventListener("clickbag", ee => {
								const Re = ee.detail.originalTarget.href;
								Re !== "" && (window.location.href = Re)
							}))
						}), _ == null || _.getStorefront().then(Y => {
							Kt(Y, ce, Fe.current)
						}, () => {}), _.on("storefrontChange", Y => {
							Kt(Y, ce, Fe.current)
						}))), window.acStore = _, I(_), $e(m(ni, {
							children: Y => m(ye, {
								acStoreInstance: Y,
								globalNavEl: ae,
								onFlyoutContentUpdate: Ie
							})
						})), S.current !== null && window.addEventListener("segmentExit", Y => {
							_ == null || _.getStorefront().then(ee => {
								Kt(ee, ce, Fe.current)
							}, () => {})
						})
					}).catch(Q => {}), () => {
						ae !== null && window.removeEventListener("segmentExit", Q => {})
					}
				}, [fe, x, f, ue, Ie, I]);
				...
				return m(ha, {
					...
					children: [m("nav", {
						id: "globalnav",
						className: `globalnav ${Ve()} ${Ce} ${lt} ${ct}`,
						lang: Nt,
						dir: t,
						ref: S,
						"aria-label": s,
						...
						children: m("div", {
							...
							children: [m("div", {
								...
								children: [m(ma, {
									...
									action: D.current !== void 0 ? D.current : ke(c.open.url, o ? y : void 0),
									input: c.input,
...
`,Fo={},$o={},To=({url:d,text:u,type:p,analyticsValue:v,flyoutItemClass:w})=>{const E=Fo[p],F=$o[p];return _("li",{className:`
$ {
  w
}
...
var So = Mi(wt.exports),
  ...
  const megaMenus = {
	/*'#presentation': [
      { title: 'Esplora competenze', links: [
        { label: 'UI Design', href: '#skills' },
        { label: 'Product Design', href: '#skills' },
        { label: 'Branding', href: '#skills' }
      ]}
    ],*/
    /*'#skills': [
      { title: 'Esplora competenze', links: [
    ...
    '#projects': [
      { title: 'Progetti', links: [
        { label: 'Progetti Mobile', href: '#projects' },
        { label: 'Progetti Desktop', href: '#projects-desktop' }
      ]},
      { title: 'Altro', links: [
        { label: 'Tutti i progetti', href: '#projects' }
      ]}
    ],
    '#publications': [
      { title: 'Altro', links: [
  ...
  function buildMegaMenu(href, columns) {
    const link = document.querySelector(`#globalheader a[href="${href}"]`);
    if (!link || link.dataset.megaInit) return;
    link.dataset.megaInit = "true";
  ...
  function initMegaMenus() {
    Object.entries(megaMenus).forEach(([href, cols]) => buildMegaMenu(href, cols));
  }

  // Overlay unico, creato una sola volta
  const overlay = document.createElement('div');
  overlay.className = 'mega-overlay';
  document.body.appendChild(overlay);

  // Delegation: funziona sempre, anche se i wrapper vengono ricreati
  ...
  document.addEventListener('mouseout', (e) => {
    const stillInside = e.relatedTarget && e.relatedTarget.closest('.mega-menu-wrapper');
    const wasInside = e.target.closest('.mega-menu-wrapper');
  ...
  const observer = new MutationObserver(initMegaMenus);
  observer.observe(header, { childList: true, subtree: true });
  setTimeout(initMegaMenus, 1000);
